...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Set motor direction using AIN1 and AIN2 pins
void setMotorDirection(uint8_t direction)
{
    switch(direction) {
        case MOTOR_FORWARD:
...
void setMotorSpeed(uint16_t speed)
{
    // Ensure speed is within bounds (0-1000)
...
int main(void)
{
  ...
  while(1) {
	...
	// Try to receive the complete message
	  if (HAL_UART_Receive(&huart1, message, MESSAGE_LENGTH, UART_TIMEOUT) == HAL_OK)
	  {
		  message[4] = '\0';
		  ...
		  if(received_value < 1800) {
			  setMotorDirection(MOTOR_BACKWARD);
			  uint16_t speed = map(received_value, 0, 1800, 1000, 0);
			  setMotorSpeed(speed);
			  sprintf(debug_msg, "BWD: ADC=%d, Speed=%d\r\n", received_value, speed);
		  }
		  else if(received_value > 2200) {
			  setMotorDirection(MOTOR_FORWARD);
			  uint16_t speed = map(received_value, 2200, 4095, 0, 1000);
			  setMotorSpeed(speed);
			  sprintf(debug_msg, "FWD: ADC=%d, Speed=%d\r\n", received_value, speed);
		  }
		  else {
			  setMotorDirection(MOTOR_STOP);
			  setMotorSpeed(0);
			  sprintf(debug_msg, "STOP: ADC=%d\r\n", received_value);
		  }

		  char debug__msg[50];
		  sprintf(debug__msg, "Timer Period: %lu\r\n", htim1.Init.Period);
		  HAL_UART_Transmit(&huart2, (uint8_t*)debug__msg, strlen(debug__msg), UART_TIMEOUT);

		  HAL_UART_Transmit(&huart2, (uint8_t*)debug_msg, strlen(debug_msg), UART_TIMEOUT);
	  }
  }
  /* USER CODE END 3 */
}
